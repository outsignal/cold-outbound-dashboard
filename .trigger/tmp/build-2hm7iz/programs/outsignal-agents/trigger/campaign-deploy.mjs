...
import {
  notifyCampaignLive,
  notifyDeploy
} from "../../../chunk-452OXXBR.mjs";
import "../../../chunk-HSFGK6GU.mjs";
import "../../../chunk-OCSVRKXM.mjs";
import "../../../chunk-OEORPP6A.mjs";
import "../../../chunk-ZVMQUHPG.mjs";
...
async function createSequenceRulesForCampaign(params) {
  ...
  const data = linkedinSequence.map((step) => ({
    ...
    triggerStepRef: step.triggerEvent === "email_sent" || !step.triggerEvent && step.position !== 1 ? step.triggerStepRef ?? `email_${step.position}` : step.triggerStepRef ?? null,
...
__name(createSequenceRulesForCampaign, "createSequenceRulesForCampaign");

// src/lib/campaigns/deploy.ts
async function withRetry(fn, maxRetries = 3, delays = [1e3, 5e3, 15e3]) {
  let lastError;
...
async function deployEmailChannel(deployId, campaignId, campaignName, workspaceSlug, emailSequence, apiToken) {
    ...
    for (const entry of leads) {
      ...
      await withRetry(
        () => ebClient.createLead({
          ...
          jobTitle: person.jobTitle ?? void 0,
          company: person.company ?? void 0
        })
      );
      leadCount++;
...
__name(deployEmailChannel, "deployEmailChannel");
async function deployLinkedInChannel(deployId, campaignId, workspaceSlug, linkedinSequence, hasEmailChannel) {
  await prisma.campaignDeploy.update({
    where: { id: deployId },
  ...
  try {
    ...
    const shouldEnqueueFirst = !hasEmailChannel || triggerEvent === "delay_after_previous";
    ...
    for (let i = 0; i < leads.length; i++) {
      ...
      if (shouldEnqueueFirst) {
        const scheduledFor = new Date(Date.now() + i * staggerMinutes * 60 * 1e3);
        await enqueueAction({
          senderId: sender.id,
          personId: person.id,
          workspaceSlug,
...
__name(deployLinkedInChannel, "deployLinkedInChannel");
async function finalizeDeployStatus(deployId, channels) {
...
__name(finalizeDeployStatus, "finalizeDeployStatus");
async function executeDeploy(campaignId, deployId) {
  await prisma.campaignDeploy.update({
    where: { id: deployId },
    data: { status: "running" }
  });
  try {
    ...
    if (hasEmail) {
      const emailSequence = campaign.emailSequence ?? [];
    ...
    if (hasLinkedIn) {
      const linkedinSequence = campaign.linkedinSequence ?? [];
    ...
    if (finalDeploy) {
      ...
      const deployStatus = finalDeploy.status;
...
__name(executeDeploy, "executeDeploy");
async function retryDeployChannel(deployId, channel) {
  const deploy = await prisma.campaignDeploy.findUniqueOrThrow({
    where: { id: deployId },
    select: {
      campaignId: true,
  ...
  const channels = JSON.parse(deploy.channels);
  const hasEmail = channels.includes("email");
  if (channel === "email") {
    await prisma.campaignDeploy.update({
      where: { id: deployId },
...
__name(retryDeployChannel, "retryDeployChannel");

// trigger/campaign-deploy.ts
var campaignDeployTask = task({
  id: "campaign-deploy",
...
//# sourceMappingURL=campaign-deploy.mjs.map
