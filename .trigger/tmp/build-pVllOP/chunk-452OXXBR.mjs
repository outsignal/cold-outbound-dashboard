...
async function notifyDeploy(params) {
  ...
  const statusLabel = params.status === "complete" ? "Complete" : params.status === "partial_failure" ? "Partial Failure" : "Failed";
  const statusEmoji = params.status === "complete" ? ":white_check_mark:" : params.status === "partial_failure" ? ":warning:" : ":x:";
  ...
  if (adminEmail) {
    try {
      ...
      if (verified.length > 0) {
        ...
        const pillColor = params.status === "complete" ? "#065f46" : params.status === "partial_failure" ? "#92400e" : "#991b1b";
        const pillBg = params.status === "complete" ? "#d1fae5" : params.status === "partial_failure" ? "#fffbeb" : "#fef2f2";
        const emailChannelRow = params.emailStatus && params.emailStatus !== "skipped" ? `<tr>
        ...
        const linkedinChannelRow = params.linkedinStatus && params.linkedinStatus !== "skipped" ? `<tr>
                      <td style="padding:10px 18px 0 18px;">
        ...
        const errorSection = params.error ? `              <!-- Error section -->
              <tr>
                <td style="padding-bottom:24px;">
                  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-collapse:collapse;">
                    <tr>
...
__name(notifyDeploy, "notifyDeploy");
async function notifyCampaignLive(params) {
  const workspace = await prisma.workspace.findUnique({
    where: { slug: params.workspaceSlug }
  });
  if (!workspace) return;
  ...
  const message = params.status === "complete" ? `Your campaign ${params.campaignName} is now live` : `Your campaign ${params.campaignName} is being launched — we're working on finalizing setup`;
...
__name(notifyCampaignLive, "notifyCampaignLive");
async function notifyInboxDisconnect(params) {
  const workspace = await prisma.workspace.findUnique({
    where: { slug: params.workspaceSlug }
  });
  if (!workspace) return;
  ...
  const headerText = hasNew ? "Inbox Disconnection Alert" : "Inbox Still Disconnected";
  const adminEmail = process.env.ADMIN_EMAIL;
  if (adminEmail && (hasNew || hasPersistent)) {
    try {
      ...
      if (verified.length === 0) return;
      {
        ...
        if (hasNew) {
          const emailRows = params.newDisconnections.slice(0, 20).map(
            (e) => `<tr>
          ...
          const overflowRow = newCount > 20 ? `<tr><td colspan="2" style="padding:8px 0;font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#a1a1aa;">...and ${newCount - 20} more</td></tr>` : "";
        ...
        if (hasPersistent) {
          const persistentRows = params.persistentDisconnections.slice(0, 20).map(
            (e) => `<tr>
                  <td style="padding:8px 0;border-bottom:1px solid #f4f4f5;">
                    <span style="font-family:'SFMono-Regular',Consolas,'Liberation Mono',Menlo,monospace;font-size:13px;color:#18181b;">${e}</span>
                  </td>
          ...
          const persistentOverflowRow = persistentCount > 20 ? `<tr><td colspan="2" style="padding:8px 0;font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#a1a1aa;">...and ${persistentCount - 20} more</td></tr>` : "";
        ...
        if (params.reconnections.length > 0) {
          const reconnRows = params.reconnections.slice(0, 10).map(
            (e) => `<tr>
                  <td style="padding:8px 0;border-bottom:1px solid #f4f4f5;">
                    <span style="font-family:'SFMono-Regular',Consolas,'Liberation Mono',Menlo,monospace;font-size:13px;color:#18181b;">${e}</span>
                  </td>
          ...
          const reconnOverflow = params.reconnections.length > 10 ? `<tr><td colspan="2" style="padding:8px 0;font-family:Arial,Helvetica,sans-serif;font-size:13px;color:#a1a1aa;">...and ${params.reconnections.length - 10} more</td></tr>` : "";
          reconnectionsHtml = `
              <tr>
                <td style="padding-bottom:20px;">
                  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="border-collapse:collapse;">
...
__name(notifyInboxDisconnect, "notifyInboxDisconnect");
async function notifySenderHealth(params) {
  const workspace = await prisma.workspace.findUnique({
    where: { slug: params.workspaceSlug }
  });
  if (!workspace) return;
  ...
  const headerText = params.severity === "critical" ? `[${workspace.name}] Sender Health Alert` : `[${workspace.name}] Sender Health Warning`;
  const reasonLabel = {
...
${params.workspacePaused ? `              <!-- Workspace paused alert -->
              <tr>
                <td style="padding-bottom:24px;">
                  <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
                    <tr>
...
${params.reassignedCount > 0 ? `              <!-- Reassignment info -->
              <tr>
                <td style="padding-bottom:24px;">
...
__name(notifySenderHealth, "notifySenderHealth");
async function sendSenderHealthDigest(params) {
  ...
  if (adminEmail) {
    try {
      ...
      if (verified.length > 0) {
        const warningRowsHtml = params.warnings.map(
          (w) => `<tr>
                <td style="padding:8px 0;border-bottom:1px solid #f4f4f5;">
...
__name(sendSenderHealthDigest, "sendSenderHealthDigest");
...
//# sourceMappingURL=chunk-452OXXBR.mjs.map
