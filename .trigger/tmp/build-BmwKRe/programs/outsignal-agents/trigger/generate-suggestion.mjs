...
init_esm();
var import_client6 = __toESM(require_default());

// src/lib/agents/runner.ts
init_esm();
async function runAgent(config, userMessage, options) {
...
init_esm();
var import_client5 = __toESM(require_default());
...
function getClient() {
  if (!_client) {
    if (!process.env.OPENAI_API_KEY) {
      throw new Error(
        "OPENAI_API_KEY environment variable is not set. Add it via: vercel env add OPENAI_API_KEY"
      );
    }
...
async function searchKnowledge(query, options) {
  ...
  if (chunkCount > 0) {
    try {
      ...
      const results = await prisma.$queryRaw`
        ...
        ${options?.tags ? import_client5.Prisma.sql`WHERE kd.tags LIKE ${"%" + options.tags + "%"}` : import_client5.Prisma.empty}
  ...
  const where = options?.tags ? { tags: { contains: options.tags } } : {};
  ...
  return scored.slice(0, limit2).map(({ title, chunk, tags }) => ({
    title,
    chunk,
    tags
  }));
...
// src/lib/agents/shared-tools.ts
var searchKnowledgeBase = tool({
  description: "Search the Outsignal knowledge base for cold email and LinkedIn outreach best practices, frameworks, templates, and guidelines. Returns semantically relevant passages. Use this to ground content in proven strategies.",
  inputSchema: external_exports.object({
    ...
    limit: external_exports.number().optional().default(8).describe("Max results (default 8)")
  }),
  execute: /* @__PURE__ */ __name(async ({ query, tags, limit: limit2 }) => {
    const results = await searchKnowledge(query, { limit: limit2, tags });
    if (results.length === 0) {
      return {
...
var researchOutputSchema = external_exports.object({
  companyOverview: external_exports.string(),
  icpIndicators: external_exports.object({
    industries: external_exports.string(),
    titles: external_exports.string(),
    companySize: external_exports.string(),
    countries: external_exports.string()
  }),
  valuePropositions: external_exports.array(external_exports.string()),
  caseStudies: external_exports.array(
  ...
  painPoints: external_exports.array(external_exports.string()),
  differentiators: external_exports.array(external_exports.string()),
  pricingSignals: external_exports.string().optional(),
  contentTone: external_exports.string(),
  suggestions: external_exports.array(external_exports.string())
});
var writerOutputSchema = external_exports.object({
  campaignName: external_exports.string(),
  channel: external_exports.enum(["email", "linkedin", "email_linkedin"]),
  emailSteps: external_exports.array(
    external_exports.object({
      position: external_exports.number(),
      subjectLine: external_exports.string(),
      subjectVariantB: external_exports.string().optional(),
      body: external_exports.string(),
      delayDays: external_exports.number(),
      notes: external_exports.string()
    })
  ).optional(),
  linkedinSteps: external_exports.array(
    external_exports.object({
      position: external_exports.number(),
      type: external_exports.enum(["connection_request", "message", "inmail"]),
      body: external_exports.string(),
      delayDays: external_exports.number(),
      notes: external_exports.string()
    })
  ).optional(),
  reviewNotes: external_exports.string(),
  creativeIdeas: external_exports.array(
    external_exports.object({
      position: external_exports.number(),
      title: external_exports.string(),
      groundedIn: external_exports.string(),
      subjectLine: external_exports.string(),
      subjectVariantB: external_exports.string().optional(),
      body: external_exports.string(),
      notes: external_exports.string()
...
var leadsOutputSchema = external_exports.object({
  action: external_exports.string(),
  summary: external_exports.string(),
  data: external_exports.unknown().optional()
});
var campaignOutputSchema = external_exports.object({
  action: external_exports.string(),
  summary: external_exports.string(),
...
var writerTools = {
  getWorkspaceIntelligence: tool({
    ...
    execute: /* @__PURE__ */ __name(async ({ slug }) => {
      ...
      return {
        ...
        websiteAnalysis: analysis ? JSON.parse(analysis.analysis) : "No website analysis available yet."
      };
    }, "execute")
  }),
  getCampaignPerformance: tool({
    description: "Get campaign performance metrics for a workspace. Use this to understand what's working and what isn't — reply rates, bounce rates, engagement data. This helps you write data-informed copy.",
    inputSchema: external_exports.object({
      workspaceSlug: external_exports.string().describe("The workspace slug")
    }),
    execute: /* @__PURE__ */ __name(async ({ workspaceSlug }) => {
      try {
        ...
        return campaigns.map((c) => ({
          ...
          reply_rate: c.emails_sent > 0 ? (c.replied / c.emails_sent * 100).toFixed(1) + "%" : "0%",
          open_rate: c.emails_sent > 0 ? (c.opened / c.emails_sent * 100).toFixed(1) + "%" : "0%",
          bounce_rate: c.emails_sent > 0 ? (c.bounced / c.emails_sent * 100).toFixed(1) + "%" : "0%"
        }));
      } catch (e) {
        return {
  ...
  getSequenceSteps: tool({
    description: "Get the actual email copy (subject lines and body text) from an existing campaign's sequence steps. Use this to study what copy has been used before and how it performed.",
    inputSchema: external_exports.object({
      workspaceSlug: external_exports.string().describe("The workspace slug"),
      campaignId: external_exports.number().describe("The campaign ID to get sequence steps for")
    }),
    execute: /* @__PURE__ */ __name(async ({ workspaceSlug, campaignId }) => {
      try {
        const client = await getClientForWorkspace(workspaceSlug);
  ...
  getExistingDrafts: tool({
    description: "Get existing email/LinkedIn drafts for a workspace and campaign. Use this to check for previous versions when revising copy.",
    inputSchema: external_exports.object({
      workspaceSlug: external_exports.string().describe("The workspace slug"),
      campaignName: external_exports.string().optional().describe("Campaign name filter")
    }),
    execute: /* @__PURE__ */ __name(async ({ workspaceSlug, campaignName }) => {
  ...
  getCampaignContext: tool({
    ...
    inputSchema: external_exports.object({
      campaignId: external_exports.string().describe("The campaign ID")
    }),
    execute: /* @__PURE__ */ __name(async ({ campaignId }) => {
      const { getCampaign } = await import("../../../operations-KNW7CRJQ.mjs");
  ...
  saveCampaignSequence: tool({
    ...
    inputSchema: external_exports.object({
      campaignId: external_exports.string().describe("The campaign ID"),
      emailSequence: external_exports.array(
        external_exports.object({
          position: external_exports.number(),
          subjectLine: external_exports.string(),
          subjectVariantB: external_exports.string().optional(),
      ...
      linkedinSequence: external_exports.array(
        external_exports.object({
          position: external_exports.number(),
          type: external_exports.enum(["connection_request", "message", "inmail"]),
          body: external_exports.string(),
      ...
      copyStrategy: external_exports.enum(["creative-ideas", "pvp", "one-liner", "custom"]).optional().describe("The copy strategy used to generate this sequence")
    }),
    execute: /* @__PURE__ */ __name(async ({ campaignId, emailSequence, linkedinSequence, copyStrategy }) => {
      const { saveCampaignSequences } = await import("../../../operations-KNW7CRJQ.mjs");
      const updated = await saveCampaignSequences(campaignId, {
        emailSequence: emailSequence ?? void 0,
        linkedinSequence: linkedinSequence ?? void 0,
        copyStrategy: copyStrategy ?? void 0
      });
      return {
        status: "saved",
  ...
  saveDraft: tool({
    description: "Save an email or LinkedIn draft to the database for review. Call this for each step in the sequence. The draft starts in 'draft' status and can be reviewed/approved later.",
    inputSchema: external_exports.object({
      workspaceSlug: external_exports.string().describe("The workspace slug"),
      campaignName: external_exports.string().describe("Campaign name"),
      channel: external_exports.enum(["email", "linkedin"]).describe("Channel: email or linkedin"),
      sequenceStep: external_exports.number().describe("Step position (1, 2, 3, etc.)"),
      subjectLine: external_exports.string().optional().describe("Email subject line (null for LinkedIn)"),
      subjectVariantB: external_exports.string().optional().describe("A/B test subject variant"),
      bodyText: external_exports.string().describe("The message body (plain text)"),
      bodyHtml: external_exports.string().optional().describe("HTML version of the body (for emails)"),
      delayDays: external_exports.number().optional().default(1).describe("Days to wait before sending this step")
    }),
  ...
  generateKBExamples: tool({
    description: "Generate draft copy examples from workspace intelligence for a given strategy. Output is formatted for admin review before ingestion into the Knowledge Base. Admin must review and approve before running ingest-document.ts CLI.",
    inputSchema: external_exports.object({
      workspaceSlug: external_exports.string().describe("The workspace slug"),
      strategy: external_exports.enum(["creative-ideas", "pvp", "one-liner"]).describe("Strategy to generate examples for"),
      count: external_exports.number().optional().default(2).describe("Number of example emails to generate (default 2)")
    }),
    execute: /* @__PURE__ */ __name(async ({ workspaceSlug, strategy, count }) => {
...
var WRITER_SYSTEM_PROMPT = `You are the Outsignal Writer Agent — an expert cold outreach copywriter specialising in email and LinkedIn campaigns that get replies.
...
var generateSuggestion = task({
  id: "generate-suggestion",
  queue: anthropicQueue,
  maxDuration: 300,
  // 5 min — Opus + KB search + multi-step tool calls can take 60-120s, plus margin
  retry: {
    maxAttempts: 2,
    factor: 2,
    minTimeoutInMs: 5e3,
  ...
  run: /* @__PURE__ */ __name(async (payload) => {
    ...
    try {
      const reply = await prisma2.reply.findUnique({
        where: { id: replyId }
      });
      if (!reply) {
      ...
      if (reply.emailBisonParentId) {
        threadReplies = await prisma2.reply.findMany({
          where: {
            workspaceSlug,
...
//# sourceMappingURL=generate-suggestion.mjs.map
