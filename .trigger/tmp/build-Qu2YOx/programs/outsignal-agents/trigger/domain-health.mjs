...
async function collectSendingDomains() {
  const senders = await prisma2.sender.findMany({
...
async function buildPriorityQueue(domains) {
  const now = /* @__PURE__ */ new Date();
  const domainHealthRecords = await prisma2.domainHealth.findMany({
  ...
  const recentSnapshots = await prisma2.bounceSnapshot.findMany({
    where: {
...
async function checkDomain(priority) {
  ...
  try {
    await prisma2.domainHealth.upsert({
      where: { domain },
      create: {
        domain,
        ...updateData
