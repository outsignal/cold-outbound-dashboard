...
export async function GET(request: Request) {
  ...
  for (const ws of workspaces.filter((w) => w.hasApiToken)) {
    const wsResult = { workspace: ws.slug, processed: 0, skipped: 0, errors: 0 };
    ...
    try {
      ...
      for (const reply of recent) {
        try {
          ...
          try {
            ...
            // Non-blocking — reply still counts as processed
          }
      ...
      console.error(`[poll-replies] Error processing workspace ${ws.slug}:`, err);
      wsResult.errors++;
    }
  }
